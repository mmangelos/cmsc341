...
class JQueue {
 ...
 private:
  ...
  // swaps nodes, helper in the recursive merge function
  void swapNodes(Node*& p1, Node*& p2);
  // swaps the left and right subtrees of the node passed
  void swapSubtrees(Node*& toSwap);
  // helper function for the recursive merging of queues
  Node* recursiveMerge(Node* p1, Node* p2);
  // preorder traversal of the queue. Helper for the dump function
  void dumpHelper(Node* currentNode) const;
  // preorder traversal of the job queue. Helper for said print function
  void printHelper(Node* currentNode) const;
  // deletes the entire heap
  void deleteHeap(Node* currentNode);
  // builds a new deep copy of a passed heap
  Node* buildHeap(Node* currentNode);
};
